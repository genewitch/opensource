#!/usr/bin/python3
#
## uses aprs.fi to get weather.
# wxreduce.out was map reduced using postgresql and a 
# 1.6 million row "zone" database and the output from
# http://wxqa.com/members.txt
# this *will* not work without wxreduce.out
# stored here for posterity.
import os
from bs4 import BeautifulSoup
from mechanize import Browser
from urllib import parse
stations = open('/var/www/fileserv.projectftm.com/wxreduce.out', 'r')
...
url = "https://aprs.fi/weather/a/" + request
page = mech.open(url)
html = page.read()
...
#oof.
print ('{"response_type": "in_channel", "text": "#### Station: ' + request + '\\n', end='')

#go through each <tr> and yank the text out. there's 2 things in the ResultSet for each row.
for c in rows[1:]:
        print (c.text + "\\n", end='')
print('[see weather charts for this station on aprs.fi](http://aprs.fi/weather/a/'+request+')\\n',end='')
